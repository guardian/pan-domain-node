...
var PandaAuthVerify = function () {
    ...
    return function (req, res, next) {
        PandaUtils.log('info', '********** TEST LOGGING STATEMENT 6  *****');
        var guAuthCookie = req.cookies[PandaUtils.settings.cookieName],
            data,
            signature;

        PandaUtils.log('info', 'guAuthCookie ' + guAuthCookie);
        ...
        if (!guAuthCookie) {
            ...
            PandaUtils.log('info', 'pandaPublicKey ' + pandaPublicKey);
            ...
            PandaUtils.log('info', 'data ' + data);
            PandaUtils.log('info', 'signature ' + signature);
            PandaUtils.log('info', PandaUtils.verifySignature(data, signature, pandaPublicKey));
...
module.exports = function (domain) {
    PandaUtils.setAwsBucket(domain);
    var pandaAuthVerify = new PandaAuthVerify();
    return {
