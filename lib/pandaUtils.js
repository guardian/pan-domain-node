...
var Utils = {
    ...
    settings: {
        ...
        awsKey: 'local.dev-gutools.co.uk.settings.public',
        awsBucket: 'pan-domain-auth-settings'
    }
};
...
Utils.verifySignature = function (message, signature, pandaPublicKey) {
    Utils.log('info', "panda " + pandaPublicKey);
    Utils.log('info', "message " + message);
    Utils.log('info', "signature " + signature);
    var t = crypto.createVerify('sha256WithRSAEncryption')
                .update(message)
...
Utils.setAwsBucket = function (bucket, key) {
    if (bucket && key) {
        Utils.settings.awsBucket = bucket;
        Utils.settings.awsKey = key;
    } else {
        Utils.log('error', 'Please provide an S3 target bucket and key for the AWS key download');
    }
};
