...
export function hashIdForKey(key: string): string {
  ...
  const unsigned = BigInt(`0x${digest.toString("hex")}`);
  const bitLength = BigInt(digest.length * 8);
  const isNegativeInTwosComplement = (digest[0] & 0x80) !== 0;
  const signed = isNegativeInTwosComplement
